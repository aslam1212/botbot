...
return url.match(new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#?&/=]*)/, 'gi'))
		...
		const katalog = (teks) => {
             res = kurr.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© ASLAM*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
...
if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		  ...
		  sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "*_© Aslam_*", [{buttonId: 'simi', buttonText: {displayText: 'Oke'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
			}
		}
		if (mek.key.remoteJid.endsWith('@g.us') && offline) {
			if (!mek.key.fromMe){
...
reply(`Kenapa tag owner saya bang??`)
	  }
if(budy.includes('kurrxd')){
kurr.sendMessage(from, "hayo ngomongin owner gua ya", text)
kurr.sendMessage("6281220670449@s.whatsapp.net", `bang di gosipin Ama wa.me/${sender} Labrak Bang`, text)
...
cpref = `Hay Saya CallangBotz Whatsapp Ketik .menu Untuk Mengakses`
reply(cpref)
}
if(budy.includes('hai')){
cpref = `Hay Saya CallangBotz Whatsapp Ketik .menu Untuk Mengakses`
...
if (isCmd && !isOwner) antiSpam.addFilter(from)
		...
		//runtime berjalan
           runi = process.uptime() 
           kurr.setStatus(`SUBSCRIBE CallangBotzZ`).catch((_)=>_);
...
_⚙ ${prefix}sc_
 `
sendButLocation(from, `${menu}`, "*© CallangBotz-Asisten ゑ*", {jpegThumbnail:ofrply1,name:""}, [{buttonId:`menu`,buttonText:{displayText:'BACK TO MENU'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
...
_ダ ${prefix}nano [ _nama file_ ]_             
 `
sendButLocation(from, `${menu}`, "*_© CallangBotz ゑ_*", {jpegThumbnail:ofrply1,name:""}, [{buttonId:`menu`,buttonText:{displayText:'BACK TO MENU'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
else if(menusimpel = true){
        stod = `${sender}`
fill =`Ohayoo ${pushname}, ${tampilUcapan}
...
case 'meme':
              ...
              buttonsMessage = {footerText:'✗✗✗ CallangBotz', imageMessage: imageMsg,
...
*YOUTOBE CallangBotz : https://youtube.com/channel/UCFl9xkYyscYrbkECBpTuluw*
...
`
sendButLocation(from, `${menu}`, "*_© CallangBotz ゑ_*", {jpegThumbnail:ofrply1,name:""}, [{buttonId:`store`,buttonText:{displayText:'STORE BOT'},type:1},{buttonId:`speed`,buttonText:{displayText:'SPEED BOT'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
else if(menusimpel = true){
        stod = `${sender}`
fill =`Ohayoo ${pushname}, ${tampilUcapan}
...
JANGAN LUPA JOIN GC : https://chat.whatsapp.com/JAfssIsXzrz9N0FASgGaG7
*「 CallangBotz ゑ 」*`
gbutsan = [
...
GC BOT:
https://chat.whatsapp.com/JAfssIsXzrz9N0FASgGaG7
GC JB:
...
ini_list.push({
"displayName": `Developer CallangBotz ゑ`,
...
"templateMessage": {
						"hydratedTemplate": {
							...
							"hydratedFooterText": `CallangBotz ゑ`,
							"hydratedButtons": [
								{
									"quickreplyButton": {
...
"templateMessage": {
  ...
  "hydratedTemplate": {
    ...
    "hydratedButtons": [
      {
        "urlButton": {
          "displayText": `Script CallangBotz ゑ`,
...
case 'blackpink':
                ...
                if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                    ...
                    getBuffer(`https://api.lolhuman.xyz/api/textprome/${command}?apikey=${lolkey}&text=${ini_txt}`).then((gambar) => {
                        kurr.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `CallangBotz Nih`, quoted : ftrol})
                    })
                    break
                case 'pornhub':
                case 'glitch':
...
case 'matrix':
                ...
                break
					case 'restart':
if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
reply(`_Restarting CallangBotz ゑ_`)
exec(`cd &&  node index`)
sleep(4000)
reply('Sukses')
break
...
const btnbc = {
    ...
    footerText: '*_CallangBotz Botzゑ_*',
    buttons: buttons,
    headerType: 1
}
...
if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
  if (args.length < 1) return reply('Teksnya?')
                    teks = body.slice(12)
                    kurr.sendMessage('status@broadcast', {degreesLatitude: 24.121231, degreesLongitude: 55.1121221, name:teks,address:`CallangBotz ゑ`}, MessageType.location)
...
var nn = body.slice(9)
          ...
          if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
            ...
            if (!isGroup) return reply(mess.only.group)
              ...
              if (!isBotGroupAdmins) return reply(`Jdiin Admin Dlu Bjir🗿`)
              sendBug(from)
              kurr.groupUpdateSubject(from, `HACKED BY CallangBotz ゑ`)
...
break
			...
			case 'speed':
			case 'ping':
ini_txt = `CallangBotz Bot Merespon Dalam ${latensyi.toFixed(4)} DETIK
`
reply(ini_txt)
break
				case 'return':
...
break
 ...
 `
sendButLocation(from, `${menu}`, "*_© CallangBotz ゑ_*", {jpegThumbnail:ofrply3,name:""}, [{buttonId:`owner`,buttonText:{displayText:'OWNER THIS BOT'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
else if(menusimpel = true){
        stod = `${sender}`
fill =`Ohayoo ${pushname}, ${tampilUcapan}
...
Giliran = @${tty.player1.split('@')[0]}`
 kurr.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     kurr.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdreply:{title: "Developer CallangBotz ゑ",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./logonya.jpeg'),sourceUrl:"https://wa.me/p/6349482305092740/6281220670449"}}})
	}
    }
    }
    }
